-- 工具数据
INSERT INTO tool_spec (name, description, type)
...
-- 工具参数数据
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
...
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
VALUES ((SELECT id FROM tool_spec WHERE name = 'runShell'), 'filePath',
        'Shell命令执行的文件夹，缺省时默认值为: 当前登录用户的主目录', 0, 'string', null);
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
VALUES ((SELECT id FROM tool_spec WHERE name = 'runShell'), 'timeout', 'Shell命令执行的超时时间/秒，缺省时默认值为:10',
        0, 'integer', null);
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
VALUES ((SELECT id FROM tool_spec WHERE name = 'readFile'), 'endLine', '结束读取行数 缺省时读取全文', 0, 'integer',
        null);
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
VALUES ((SELECT id FROM tool_spec WHERE name = 'readFile'), 'filePath', '文件路径', 1, 'string', null);
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
VALUES ((SELECT id FROM tool_spec WHERE name = 'readFile'), 'startLine', '开始读取行数 缺省时读取全文', 0, 'integer',
        null);
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
VALUES ((SELECT id FROM tool_spec WHERE name = 'writeFile'), 'filePath', '写入文件路径', 1, 'string', null);
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
VALUES ((SELECT id FROM tool_spec WHERE name = 'writeFile'), 'content', '写入文件内容，多行用系统换行符分割', 1,
        'string', null);
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
VALUES ((SELECT id FROM tool_spec WHERE name = 'writeFile'), 'isAppend', '是否为追加模式 ture-追加内容 false-替换内容',
        1, 'boolean', null);
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
VALUES ((SELECT id FROM tool_spec WHERE name = 'getRemainingTime'), 'nowDateStr', '当前时间,格式为yyyy-MM-dd HH:mm:ss',
        1, 'string', null);
INSERT INTO tool_spec_param (tool_spec_id, name, description, required, type, enum_values)
VALUES ((SELECT id FROM tool_spec WHERE name = 'getRemainingTime'), 'unit', '时间单位', 0, 'enums',
        'HOUR,MINUTE,SECOND');